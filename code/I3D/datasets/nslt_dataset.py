...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    ...
    print("The total frames is ", total_frames)
    count = 0
    vidcap.set(cv2.CAP_PROP_POS_FRAMES, start)
    offset = 0
    failure_count = 0  # Counter for consecutive failures
    while offset < min(num, int(total_frames - start)):
        ...
        if not success:
            print("Failed to read frame ", offset, " from video ", video_path)
            failure_count += 1
            if failure_count > 3:  # If more than 3 consecutive failures, skip the video
                print("Skipping video due to multiple consecutive failures")
                break
            continue

        # Reset failure count if successful frame read
        failure_count = 0
        ...
        # pick interval of 5 frames for the next frame
        # offset += 2
        # vidcap.set(cv2.CAP_PROP_POS_FRAMES, start + offset)
